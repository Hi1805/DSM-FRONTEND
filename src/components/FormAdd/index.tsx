...
import { ProfileTemplate, ClassAttributes, GlobalContext } from "../../services";
import "./FormAdd.scss";
...
export const FormAdd = (props: FormAddProps) => {
    const { status, turnOffForm } = props;

    const { handleAdd, typeTab } = React.useContext(GlobalContext);
    const {
        register,
        handleSubmit,
        formState: { errors },
    } = useForm();
    const [chooseGradeState, setChooseGradeSate] = React.useState<number>();
    ...
    React.useEffect(() => {
        (() => {
            const newInfoGrade = classesState.find((item) => item.id === chooseGradeState) || {
                total: 0,
                id: 0,
                values: []
            };
    ...
    const onSubmit = (data: ProfileTemplate) => {
        ...
        if (new Date(date_of_birth).getFullYear() > YEAR_LAST || new Date(date_of_birth).getFullYear() < YEAR_FIRST) {
        ...
        toast.promise(
            handleAdd(infoGrade, data).then(() => {
                updateTotalGrade();
            }),
            {
                pending: `Add ${typeTab} is pending 😙😙😙`,
                success: `Add  ${typeTab} resolved 👌👌👌`,
    ...
    const renderForm = () => {
        return (<React.Fragment>
            <div className="td-form-add__title">
                <span>{`Add a ${startCase(typeTab)}`}</span>
            ...
            <div className="td-form-add__body" >
                ...
                <div className="container-fluid row  flex-wrap justify-content-between mt-4">
                    ...
                    <div className="td-form-add__body__form-input">
                        ...
                        <input {...register('date_of_birth', { required: true })} id="date_of_birth" placeholder="example: Huy" type="date" className="form-control"></input>
                ...
                <div className="container-fluid row  flex-wrap justify-content-between mt-4">
                    <div className="td-form-add__body__form-input">
                        ...
                        <select {...register('grade', { pattern: regexNumber })} className="form-select form-control"
                            onChange={(e) => {
                                setChooseGradeSate(toNumber(e.target.value))
                            }}
                        >
                            <option selected>Select grade</option>
                            {classesState.map((Class) => <option key={Class.id} value={toString(Class.id)}>{Class.id}</option>)}
                        </select>
                    ...
                    <div className="td-form-add__body__form-input">
                        ...
                        <select {...register('class')} className="form-select form-control" aria-label="">
                            ...
                            {infoGrade.values.map((Class) => <option key={Class} value={Class}>{Class}</option>)}
                        </select>
                ...
                <div className="container-fluid row mt-4">
                    <div className="form-input w-100">
                        ...
                        <input {...register('address', { required: true })} id="address" placeholder="example: Da Nang , Viet Nam" type="text" className="form-control"></input>
